...
export async function autoAssignForBookingStaffModel(bookingId: string) {
  ...
  const created: { staffId: string; bookingItemId: string }[] = [];
  ...
  // Items that need Kirtan or Path (based on category or min requirements)
  ...
  // -------- KIRTAN: pick 3 least-loaded Kirtanis (no jathas needed) --------
  if (kirtanItems.length) {
    ...
    const group = orderedK.slice(0, 3);
    ...
    if (group.length === 3) {
      for (const item of kirtanItems) {
        ...
        group.forEach((id) =>
          created.push({ staffId: id, bookingItemId: item.id })
        );
      }
    } else {
      for (const item of kirtanItems) {
        shortages.push({
          itemId: item.id,
          role: 'KIRTAN',
          needed: 3 - group.length,
        });
      }
    }
  }

  // -------- PATH: Granthi → least-loaded PATH-capable sevadars --------
  if (pathItems.length) {
    // Prefer Granthi if free
    const granthi = await prisma.staff.findFirst({
      where: {
        isActive: true,
        skills: { has: 'PATH' },
        name: { equals: 'Granthi', mode: 'insensitive' },
      },
      select: { id: true },
    });
    const granthiFree = granthi && !busy.has(granthi.id) ? granthi.id : null;

    // Global PATH-capable pool
    ...
    const orderedP = await orderStaffByWeeklyCategoryLoad(
      freeP,
    ...
    for (const item of pathItems) {
      ...
      // 1) Granthi first (assign once per item if available)
      if (granthiFree && remaining > 0) {
        ...
        created.push({ staffId: granthiFree, bookingItemId: item.id });
        remaining--;
      }

      // 2) Least-loaded PATH-capable for the rest
      if (remaining > 0) {
