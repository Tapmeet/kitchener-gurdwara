...
export default async function Page({
  ...
  const week =
    typeof params?.week === 'string' ? parseISO(params.week) : new Date();
  const ws = isValid(week)
  ...
  const assignments = staffIds.length
    ? await prisma.bookingAssignment.findMany({
        where: {
          staffId: { in: staffIds },
          OR: [
            // windowed shifts: [a.start, a.end) overlaps [ws, we)
            { AND: [{ start: { lt: we } }, { end: { gt: ws } }] },
            // unwindowed (null start/end) → fall back to booking overlap
            {
              AND: [
                { start: null },
                { end: null },
                { booking: { start: { lt: we }, end: { gt: ws } } },
              ],
            },
          ],
    ...
    <div className='p-6 space-y-6'>
      <div className='flex items-center justify-between gap-4 flex-wrap'>
        <h1 className='text-xl font-semibold'>
          Staff Schedule (Week of {format(ws, 'MMM d, yyyy')})
        </h1>
        <form
        ...
        >
          <input
            ...
            defaultValue={format(ws, 'yyyy-MM-dd')}
          />
          <select
            name='jatha'
            className='border rounded px-2 py-1 text-sm'
